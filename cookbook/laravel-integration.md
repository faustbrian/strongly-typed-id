...
Attributes provide the same functionality as casts with more flexibility for custom logic.
...
For ULIDs, use `char(26)` or `string(26)`:
...
Now IDs generate automatically:
...
$user->id; // Automatically generated UserId
```
...
class Organization extends Model
{
    ...
    {
        return [
            'id' => OrganizationId::asEloquentCast(),
        ];
    }
...
$organization = $user->organization; // Organization model
...
$userData->organizationId; // OrganizationId instance
...
Create test helpers for working with IDs:
...
class UserTest extends TestCase
{
    ...
    public function test_user_relationships(): void
    {
        ...
        $this->assertTrue($user->id->equals($order->user_id));
        $this->assertTrue($user->id->equals($order->user->id));
    }
}
```
